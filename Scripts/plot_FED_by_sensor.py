...
for chart in FED_dict['Experiment_01'].columns:
	mean_ls = []
	if 'rate' in chart:
		continue
	fig = plt.figure()
	...
	for experiment in test_des.index.values:
		...
		data = data_df[chart]
		if 'Temp' in chart:
			data = data_df[chart].rolling(window=5, center=True).mean()		
...
print('---------------------------------------------------------------')
output_dir = '../Figures/rate_by_FED_location/'
if not os.path.exists(output_dir):
	os.makedirs(output_dir)
for chart in FED_dict['Experiment_01'].columns:
	if 'rate' in chart:
	...
	##PLOT FED RATE OF CHANGE by attack type##
...
for group in test_groups.groups:
	print(group)
	output_dir = '../Figures/rate_by_FED_location/'+group + '/'
	if not os.path.exists(output_dir):
		os.makedirs(output_dir)
	#define name of "water on" event
	if group =='Transitional':
