...
for f in os.listdir(data_location):
	if f.endswith('.csv'):
			...
			if FEDs_df.loc[Test_Name,'Attack Type']=='Transitional':
				Water_on=(datetime.datetime.strptime(Events['Time']['Water in Window'], '%Y-%m-%d-%H:%M:%S')-Ignition_mdy).total_seconds()
			else:
				Water_on=(datetime.datetime.strptime(Events['Time']['Nozzle FF Reaches Hallway'], '%Y-%m-%d-%H:%M:%S')-Ignition_mdy).total_seconds()
			Water.append(Water_on)
			...
			for loc in channels.index.values:
				...
				try:
					fig = figure()
					plot(gas_time,FED_cum,'k',linewidth=3)
					ax1 = gca()
					xlabel('Time (s)', fontsize=20)
					ylabel('FED', fontsize=20)
					xticks(fontsize=16)
					yticks(fontsize=16)
					...
					for i in range(len(Events.index.values)):
						...
						if EventTime[i]<max(gas_time):
							Event_Names.append(Events.index.values[i])
							Tick_Times.append(EventTime[i])

					ax1.set_xlim(0,1.1*max(gas_time))
					ax1.set_ylim(0,1.1*max(FED_cum))
					ax3=ax1.twiny()
